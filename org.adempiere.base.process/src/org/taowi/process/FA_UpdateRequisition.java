...
public class FA_UpdateRequisition extends SvrProcess {
			...
			try {
				...
				while (rs.next()) {
					...
					if (C_Charge_ID > 0 && M_Product_ID == 0) {
						...
						if(!req.isTrackAsAsset()){
							req.setIsTrackAsAsset(true);
							req.saveEx();
						}
						
						reqLine.setIsTrackAsAsset(true);
						reqLine.saveEx();
	...
	public String updateOrder(int C_Order_ID) {
		...
		sqlUpdateOrd.append("UPDATE C_Order SET isTrackAsAsset = 'Y' ");
		sqlUpdateOrd.append(" WHERE C_Order_ID = ?");
		sqlUpdateOrd.append(" AND isTrackAsAsset = 'N' ");

		DB.executeUpdateEx(sqlUpdateOrd.toString(),
	...
	public String updateOrderLine(int C_OrderLine_ID) {
		...
		sqlUpdateOrdLine.append("UPDATE C_OrderLine SET isTrackAsAsset = 'Y' ");
		sqlUpdateOrdLine.append(" WHERE C_OrderLine_ID = ?");
		sqlUpdateOrdLine.append(" AND isTrackAsAsset = 'N' ");

		DB.executeUpdateEx(sqlUpdateOrdLine.toString(),
				new Object[] { C_OrderLine_ID }, get_TrxName());
		...
		try {
			...
			while (rsMatchPO.next()) {
				...
				DB.executeUpdateEx(sqlUpdate.toString(),
						new Object[] { M_InOut_ID }, get_TrxName());
				DB.executeUpdateEx(sqlUpdateLine.toString(),
						new Object[] { M_InOutLine_ID }, get_TrxName());
	...
	public String updateInvoice(int C_InOutLine_ID) {
		...
		StringBuilder sqlUpdate = new StringBuilder();
		sqlUpdate.append("UPDATE C_Invoice SET isTrackAsAsset = 'Y' ");
		sqlUpdate.append(" WHERE C_Invoice_ID = ?");
		sqlUpdate.append(" AND isTrackAsAsset = 'N' ");

		StringBuilder sqlUpdateLine = new StringBuilder();
		sqlUpdateLine.append("UPDATE C_InvoiceLine SET isTrackAsAsset = 'Y' ");
		...
		try {
			...
			while (rsMatchInv.next()) {
				...
				DB.executeUpdateEx(sqlUpdate.toString(),
						new Object[] { M_Invoice_ID }, get_TrxName());
				DB.executeUpdateEx(sqlUpdateLine.toString(),
						new Object[] { M_InvoiceLine_ID }, get_TrxName());
