...
import org.taowi.model.MAgingDaily;
...
public class AgingDaily extends SvrProcess
{
	...
	private int			p_DaysDue = -99999;
	private int			p_DaysDueTo = 99999;
	...
	protected void prepare()
	{
		...
		for (int i = 0; i < para.length; i++)
		{
			...
			{
				p_DaysDue = para[i].getParameterAsInt();
				p_DaysDueTo = para[i].getParameter_ToAsInt();
			}
			else
	...
	protected String doIt() throws Exception
	{
		...
		sql.append(" AND oi.DaysDue BETWEEN " + p_DaysDue + " AND " + p_DaysDueTo); //GIO-2353 add parameter validation DaysDue @Febrian
		...
		try
		{
			...
			while (rs.next())
			{
					...
					|| C_InvoicePaySchedule_ID != aging.getC_InvoicePaySchedule_ID())
				{
					...
					aging = new MAgingDaily (getCtx(), AD_PInstance_ID, p_StatementDate, 
