...
*/
@org.idempiere.ui.zk.annotation.Form(name = "org.compiere.apps.form.ArchiveViewerWorkflow")
public class WArchiveViewerWorkflow extends Archive implements IFormController, EventListener<Event>
{
	private static final String ONCLOSE_TIMESTAMP_ATTR = "onclose.timestamp";

	private class WArchiveViewerWorkflowForm extends CustomForm
	{
		/**
		 * generated serial id
		 */
	...
	/** DocumentNo field of viewer tab */
	private Textbox documentNoField = new Textbox();

	private Label dateLabel = new Label(Msg.translate(Env.getCtx(), "Date"));
	/** Date field of viewer tab */
	private Datebox dateField = new Datebox();

	private Label bPartnerLabel2 = new Label(Msg.translate(Env.getCtx(), "C_BPartner_ID"));
	/** Business Partner field of viewer tab */
	private Textbox bPartnerNameField = new Textbox();

	private Label historyLabel = new Label(Msg.translate(Env.getCtx(), "History"));
	/** History field of viewer tab */
	private Html historyField = new Html();
	 ...
	 * Default constructor
	 */
	public WArchiveViewerWorkflow()
	{
		if (log.isLoggable(Level.INFO)) log.info("");

		form = new WArchiveViewerWorkflowForm();
	...
	private void zkInit() throws Exception
	{
		...
		row = new Row();
		rows.appendChild(row);
		row.appendCellChild(documentNoLabel, 3);
		ZKUpdateUtil.setHflex(documentNoLabel, "1");

		row = new Row();
		rows.appendChild(row);
		row.appendCellChild(documentNoField, 3);
		ZKUpdateUtil.setHflex(documentNoField, "1");
		documentNoField.setReadonly(true);

		row = new Row();
		rows.appendChild(row);
		row.appendCellChild(dateLabel, 3);
		ZKUpdateUtil.setHflex(dateLabel, "1");

		row = new Row();
		rows.appendChild(row);
		row.appendCellChild(dateField, 3);
		ZKUpdateUtil.setHflex(dateField, "1");
		dateField.setReadonly(true);

		row = new Row();
		rows.appendChild(row);
		row.appendCellChild(bPartnerLabel2, 3);
		ZKUpdateUtil.setHflex(bPartnerLabel2, "1");

		row = new Row();
		rows.appendChild(row);
		row.appendCellChild(bPartnerNameField, 3);
		ZKUpdateUtil.setHflex(bPartnerNameField, "1");
		bPartnerNameField.setReadonly(true);

		row = new Row();
		rows.appendChild(row);
		row.appendCellChild(historyLabel, 3);
		ZKUpdateUtil.setHflex(historyLabel, "1");

		row = new Row();
		rows.appendChild(row);
		row.appendCellChild(historyField, 3);
		ZKUpdateUtil.setHflex(historyField, "1");
	...
	private void updateVDisplay (boolean next)
	{
		...
		if (m_archives.length == 0)
		{
			...
			nameField.setText("");
			documentNoField.setText("");
			dateField.setValue(null);
			bPartnerNameField.setText("");
			historyField.setContent("");
		...
		// Populate new fields: DocumentNo, Date, BusinessPartner, History
		String documentNo = "";
		...
		try
		{
			int AD_Table_ID = ar.getAD_Table_ID();
			int Record_ID = ar.getRecord_ID();
			...
			if (AD_Table_ID > 0 && Record_ID > 0)
			{
				...
				if (po != null)
				{
					...
					// Get DateDoc or DateInvoiced or Date Ordered or Created date
					idx = po.get_ColumnIndex("DateDoc");
					if (idx >= 0)
						docDate = (java.util.Date) po.get_Value("DateDoc");
					else
					{
						idx = po.get_ColumnIndex("DateInvoiced");
						if (idx >= 0)
							docDate = (java.util.Date) po.get_Value("DateInvoiced");
						else
						{
							idx = po.get_ColumnIndex("DateOrdered");
							if (idx >= 0)
								docDate = (java.util.Date) po.get_Value("DateOrdered");
							else
								docDate = po.getCreated();  // Fallback to created date
						}
					}
					...
					// Get workflow history if this document has workflow activities
					MWFActivity[] activities = MWFActivity.get(Env.getCtx(), AD_Table_ID, Record_ID, false);
					if (activities != null && activities.length > 0)
					{
						StringBuilder historyHtml = new StringBuilder("<div style='overflow-y:scroll;height: 150px; border: 1px solid #7F9DB9;'>");
						for (MWFActivity activity : activities)
						{
							historyHtml.append(activity.getHistoryHTML());
						}
						historyHtml.append("</div>");
						history = historyHtml.toString();
					}
				}
			}
		}
		...
		{
			log.log(Level.WARNING, "Error getting document info for archive " + ar.getAD_Archive_ID(), e);
		}

		documentNoField.setText(documentNo);
		dateField.setValue(docDate);
		bPartnerNameField.setText(bPartnerName);
		historyField.setContent(history);
	...
	/**
	 * Load specific archives by their IDs and properly initialize the viewer
	 * @param archiveIDs array of archive IDs to load
	 */
	public void loadArchivesByIDs(int[] archiveIDs) {
		if (archiveIDs == null || archiveIDs.length == 0) {
			m_archives = new MArchive[0];
			return;
		}

		// Build WHERE clause with archive IDs
		StringBuilder whereClause = new StringBuilder(" AND AD_Archive_ID IN (");
		for (int i = 0; i < archiveIDs.length; i++) {
			if (i > 0)
				whereClause.append(",");
			whereClause.append(archiveIDs[i]);
		}
		whereClause.append(")");

		// Get archives using the where clause
		m_archives = MArchive.get(Env.getCtx(), whereClause.toString(), null);

		// Display the first archive
		m_index = 0;

		// Switch to viewer tab (not query tab since we're directly loading results)
		if (showQuery && tabbox != null)
			tabbox.setSelectedIndex(1);
