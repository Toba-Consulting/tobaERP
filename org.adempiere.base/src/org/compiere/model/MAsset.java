...
public class MAsset extends X_A_Asset {
	...
	public MAsset(MInOut mInOut, MInOutLine sLine, int deliveryCount) {
		...
		setAD_Org_ID(mInOut.getAD_Org_ID());
		setIsOwned(false);
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if (newRecord)
		{
			...
			{			
				...
				assetwk.setA_Salvage_Value(assetgrpacct.getA_Salvage_Value());
	...
	public void changeStatus(String newStatus, Timestamp date)
	{
		...
		if(newStatus.equals(A_ASSET_STATUS_Preservation) || newStatus.equals(A_ASSET_STATUS_Disposed) ) {
	...
	public MDepreciationExp[] getDepExp(int RecordID) { 
		 
		final String whereClause = COLUMNNAME_A_Asset_ID+ "=?"; 
		List<MDepreciationExp> list = new Query(getCtx(), 
				MDepreciationExp.Table_Name, whereClause, get_TrxName()) 
				.setParameters(RecordID) 
				.setOnlyActiveRecords(true) 
				.list(); 
		// 
		MDepreciationExp[] depExp = new MDepreciationExp[list.size()]; 
		list.toArray(depExp); 
		return depExp; 
	} 
	 
	public MAssetAcct[] getAssetAcct(int RecordID) { 
		final String whereClause = COLUMNNAME_A_Asset_ID+ "=?"; 
		List<MAssetAcct> list = new Query(getCtx(), 
				MAssetAcct.Table_Name, whereClause, get_TrxName()) 
				.setParameters(RecordID) 
				.setOnlyActiveRecords(true) 
				.list(); 
		// 
		MAssetAcct[] assetAcct = new MAssetAcct[list.size()]; 
		list.toArray(assetAcct); 
		return assetAcct; 
	} 
	 
	public MDepreciationWorkfile[] getWorkfile(int RecordID) { 
		final String whereClause = COLUMNNAME_A_Asset_ID+ "=?"; 
		List<MDepreciationWorkfile> list = new Query(getCtx(), 
				MDepreciationWorkfile.Table_Name, whereClause, get_TrxName()) 
				.setParameters(RecordID) 
				.setOnlyActiveRecords(true) 
				.list(); 
		// 
		MDepreciationWorkfile[] assetwks = new MDepreciationWorkfile[list.size()]; 
		list.toArray(assetwks); 
		return assetwks; 
	} 
	 
	public boolean isSecondAddition(){ 
		String where = "A_Asset_ID="+getA_Asset_ID() + " AND " +  MAssetAddition.COLUMNNAME_A_CapvsExp + "='" + MAssetAddition.A_CAPVSEXP_Capital + "'"; 
		int[] assetAdds = new Query(getCtx(), MAssetAddition.Table_Name, where, get_TrxName()) 
			.setClient_ID()	 
			.setOnlyActiveRecords(true) 
			.getIDs(); 
		 
		if(assetAdds.length > 1) 
			return true; 
		 
		return false; 
	} 
	 
	public Timestamp getDateFirstAddition(){ 
		String where = "A_Asset_ID="+getA_Asset_ID(); 
		 
		int A_Asset_Addition_ID = new Query(getCtx(), MAssetAddition.Table_Name, where, get_TrxName()) 
			.setClient_ID() 
			.setOnlyActiveRecords(true) 
			.setOrderBy(MAssetAddition.COLUMNNAME_DateAcct) 
			.firstId(); 
		 
		MAssetAddition assetAdd = new MAssetAddition(getCtx(), A_Asset_Addition_ID, get_TrxName()); 
		 
		return assetAdd.getDateAcct(); 
	}
}
